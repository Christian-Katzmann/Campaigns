...
import { CAMPAIGN_FILE_EXTENSION } from '../public/lib/campaign-file.mjs';
...
ACCEPTANCE:
- The outcome and constraints are explicit enough to guide implementation.
OPEN QUESTIONS:
...
ACCEPTANCE:
- The stated outcome is implemented and practically verified.
OPEN QUESTIONS:
...
export async function createCampaignScaffold({ name, projectPath }) {
  ...
  return writeCampaignFile({
    markdown: campaignTemplate(title),
    projectPath,
    slug,
    title,
  });
...
export async function createCampaignFromMarkdown({ markdown, name, projectPath }) {
  if (typeof markdown !== 'string' || !markdown.trim()) {
    throw scaffoldError(400, 'Campaign markdown is required.');
  }
  const { title, slug } = normalizeCampaignName(name);
  return writeCampaignFile({ markdown, projectPath, slug, title });
}

async function writeCampaignFile({ markdown, projectPath, slug, title }) {
  ...
  const filePath = path.join(campaignsDir, `${slug}${CAMPAIGN_FILE_EXTENSION}`);
