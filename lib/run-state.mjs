...
const EVENT_STATUS_TRANSITIONS = Object.freeze({
  ...
  force_merged_unreviewed: {
    from: ['awaiting_review', 'reviewing', 'reworking', 'halted'],
    to: ['force_merged'],
  },
  stopped_by_user: {
    from: [
  ...
  recovery_started: {
    from: ['blocked', 'failed', 'halted', 'stopped'],
    to: ['recovering'],
  },
...
export function transitionRunState(state, input) {
  ...
  if (eventName === 'run_started') {
    ...
    requireRunStatus(next, eventName, ['awaiting_review', 'reviewing', 'reworking', 'halted']);
    ...
    next.review.verdict = null;
    next.review.reasons = normalizeReasons(input.reasons ?? []);
  } else if (eventName === 'stopped_by_user') {
    ...
    next.recovery = null;
    setRunStatus(next, 'stopped', at);
  } else if (eventName === 'recovery_started') {
    requireRunStatus(next, eventName, ['blocked', 'failed', 'halted', 'stopped']);
    next.recovery = {
      from_status: next.run.status,
      step_id: stringOrNull(input.step_id ?? next.cursor.step_id),
      started_at: at,
    };
...
function normalizeLocation(location, label, includeBranch) {
  ...
  if (includeBranch) result.branch = requireString(location.branch, `${label}.branch`);
  return result;
}
...
function validateRecovery(recovery, add) {
  ...
  if (!['blocked', 'failed', 'halted', 'stopped'].includes(recovery.from_status)) {
...
function validateCrossFieldInvariants(state, add) {
  ...
  if (['awaiting_review', 'reviewing', 'reworking', 'completed', 'merged', 'force_merged'].includes(state.run.status)) {
    const unfinished = state.steps.filter((step) => !['completed', 'skipped'].includes(step.status));
