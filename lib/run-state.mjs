...
const EVENT_STATUS_TRANSITIONS = Object.freeze({
  ...
  human_review_approved: { from: ['awaiting_human_review'], to: ['completed'] },
  final_review_halted: {
...
export function upgradeRunState(state) {
  if (!isPlainObject(state) || ![1, 2, 3, 4, 5, 6, 7, 8].includes(state.schema_version)) return state;
  ...
  if (next.schema_version === 8) {
    next.config.max_cost_usd ??= null;
...
export function transitionRunState(state, input) {
  ...
  if (eventName === 'run_started') {
  ...
  } else if (eventName === 'check_failed' || eventName === 'check_fix_finished') {
    ...
    next.review.raw_tags = [];
    next.review.findings = [];
  } else if (eventName === 'final_review_approved' || eventName === 'campaign_completed' || eventName === 'human_review_approved') {
    requireRunStatus(next, eventName, eventName === 'human_review_approved' ? ['awaiting_human_review'] : ['reviewing']);
    requireAllStepsDone(next, eventName);
    ...
    next.review.raw_tags = normalizeReasons(input.raw_tags ?? []);
    next.review.findings = normalizeReviewFindings(input.findings ?? []);
    next.review.review_path = eventName === 'human_review_approved'
      ? absolutePathOrNull(next.review.review_path, `${eventName}.review_path`)
      : requireAbsolutePath(input.review_path, `${eventName}.review_path`);
...
function normalizeConfig(config) {
  ...
  return {
    ...
    max_cost_usd: config.max_cost_usd == null
      ? null
      : requirePositiveNumber(config.max_cost_usd, 'config.max_cost_usd'),
...
function appendHistory(state, input, event, fromStatus, toStatus, at) {
  ...
  if (event === 'human_review_approved') {
    details.approved_by = 'human';
    details.approved_via = typeof input.approved_via === 'string' && input.approved_via
      ? input.approved_via
      : 'api';
    details.cause = 'reviewer_unavailable';
  }
  ...
  if ([
    ...
    details.verdict = input.verdict ?? (
      ['final_review_approved', 'human_review_approved', 'campaign_completed'].includes(event) ? 'APPROVED' : null
    );
...
function validateConfig(config, add) {
  ...
  if (!(config.max_cost_usd == null || positiveNumber(config.max_cost_usd))) {
    add('config.max_cost_usd must be null or a positive number');
  }
