...
export function redactAndCapText(value, maxCharacters) {
  ...
  const marker = `[TRUNCATED: output capped at ${maxCharacters} characters]\n`;
  if (marker.length >= maxCharacters) return marker.slice(0, maxCharacters);
  return `${marker}${redacted.slice(-(maxCharacters - marker.length))}`;
}
