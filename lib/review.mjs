...
export async function runFinalReview(options) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    const logPath = path.join(paths.logsDir, `review-${attempt}.log`);
    const outputPath = path.join(paths.logsDir, `review-${attempt}-last-message.md`);
...
async function writeReviewArtifact(review) {
  ...
  await writeFileAtomic(review.review_path, body);
}
