...
import { createLiveOutputTransport } from './live-output.mjs';
...
export async function runRunnerInvocation(invocation, options) {
  ...
  const liveOutput = options.liveOutput
    ? await createLiveOutputTransport(options.liveOutput)
    : null;
  ...
  child.stdin.on('error', (error) => {
    // The child may exit or be cancelled before its prompt reaches the pipe.
    // Its exit status remains the authoritative result in that race.
    if (!isClosedChildInputError(error)) stdinError = error;
  });
  const closed = new Promise((resolve) => {
    let spawnError = null;
    child.on('error', (error) => { spawnError = error; });
    child.on('close', (exitCode, signal) => {
      const invocationError = spawnError ?? stdinError;
      const signalExitCode = signal === 'SIGINT' ? 130 : signal === 'SIGTERM' ? 143 : 1;
      resolve({
        exitCode: invocationError ? 127 : (exitCode ?? signalExitCode),
        stdout,
        stderr: invocationError ? `${stderr}${stderr ? '\n' : ''}${invocationError.message}` : stderr,
      });
    });
  });
  const abort = () => {
...
function isClosedChildInputError(error) {
