...
export async function rollbackCampaign(campaignFile, options = {}) {
  ...
  try {
    ...
    if (state.rollback.status === 'git_completed') {
      const markdownCommitOid = await applyMarkdownRollback(
        state,
        paths.statePath,
        campaignPath,
      );
...
async function applyMarkdownRollback(state, statePath, campaignPath) {
  ...
  if (changed) {
    await writeRunDocumentTransition({
      ...
      kind: 'rollback_checkboxes_reset',
      stepId: state.rollback.boundary_step_id,
    });
  }
