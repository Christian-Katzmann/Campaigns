...
function allowedChildren(prefix) {
  ...
  if (dotted === '') {
    return new Set(['schemaVersion', '_comment', 'defaultRunner', 'watchdog', 'run', 'review', 'runners']);
  }
  if (dotted === 'watchdog') return new Set(['minimum_runtime_ms', 'stall_window_ms']);
  if (dotted === 'run') {
    return new Set(['repoRoot', 'branch', 'max_steps_per_run', 'max_run_minutes', 'stop_grace_ms']);
  }
  if (dotted === 'review') return new Set(['maxFixAttempts', 'forceMergeUnreviewed']);
  if (dotted === 'runners') return 'any';
  if (/^runners\.[^.]+$/.test(dotted)) {
    return new Set(['binary', 'args', 'prompt', 'defaults', 'effortMap', 'environment', 'completion']);
  }
