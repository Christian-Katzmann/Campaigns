...
//   <petsDir>/<pet-id>/pet.json          { id, displayName, description, spriteVersionNumber, spritesheetPath }
//   <petsDir>/<pet-id>/spritesheet.webp  v1: 1536x1872 (8x9), v2: 1536x2288 (8x11)

// Codex sprite atlas contracts. A missing manifest version is v1 for backwards
// compatibility; newly hatched pets declare v2 and include two look rows.
export const PET_SPRITES = Object.freeze({
  1: Object.freeze({
    width: 1536,
    height: 1872,
    columns: 8,
...
export function spriteGridForVersion(spriteVersionNumber) {
  return PET_SPRITES[spriteVersionNumber] ?? null;
}

// Kro ships as three small state illustrations rather than a generated atlas.
// A valid user package always wins; this descriptor is only the final fallback.
export const BUNDLED_KRO = Object.freeze({
  id: 'kro',
  displayName: 'Kro',
  description: 'A small black bird keeping watch over your campaigns.',
  source: 'bundled',
  renderMode: 'states',
  stateAssets: Object.freeze({
    idle: '/assets/kro/idle.svg',
    working: '/assets/kro/working.svg',
    'needs-attention': '/assets/kro/attention.svg',
  }),
});
...
export async function readPetManifest(petsDir, id) {
  ...
  if (!spriteGridForVersion(spriteVersionNumber)) return null;
...
// When no usable package exists, the shipped Kro descriptor makes the companion
// useful in a clean HOME with no Codex setup.
