...
function nudgeClaudeAutomateState(filePath, mode) {
  const slug = campaignFileStem(filePath);
  const modeFlags = {
    continue: ['--continue'],
    skip: ['--skip'],
...
function engineProvider() {
  return {
    ...
    async getState(filePath, { summary = false, registryId = null } = {}) {
      ...
      const rollback = buildEngineRollback(stateData, runtime);
      ...
      return {
        ...
        review: {
          ...
          reviewer_runner: stateData.review.reviewer_runner,
          reviewer_family: stateData.review.reviewer_family,
          reviewer_ladder_tier: stateData.review.reviewer_ladder_tier,
        },
...
async function matchEngineStateCandidate(statePath, markdownPath, registryId) {
  try {
    const stateData = await readRunState(statePath);
...
export function deriveEngineRuntime(stateData, lock) {
  ...
  const activeStatus = ['running', 'reviewing', 'reworking', 'rolling_back', 'recovering'].includes(runStatus);
  if (activeStatus) {
    return livePid
  ...
  if (runStatus === 'rollback_conflict') return { status: 'rollback_conflict', isActive: false };
...
function buildEngineRollback(stateData, runtime) {
  ...
  const targets = [];
  for (const [index, step] of stateData.steps.entries()) {
    if (!['completed', 'skipped'].includes(step.status)) continue;
    let boundaryIndex = index;
    if (step.parallel_group) {
      boundaryIndex = Math.max(...step.parallel_group.step_ids.map((stepId) => (
        stateData.steps.findIndex((candidate) => candidate.id === stepId)
      )));
    }
    const resetSteps = stateData.steps.slice(boundaryIndex + 1)
      .filter((candidate) => ['completed', 'skipped'].includes(candidate.status))
      .map((candidate) => candidate.id);
    if (resetSteps.length === 0) continue;
    targets.push({
      step_id: step.id,
      boundary_step_id: stateData.steps[boundaryIndex]?.id ?? step.id,
      reset_steps: resetSteps,
      includes_parallel_group: boundaryIndex !== index,
    });
  }
  return {
    available: !runtime.isActive && !disallowed.has(stateData.run.status) && targets.length > 0,
    targets,
    active: stateData.rollback ? {
      id: stateData.rollback.id,
      status: stateData.rollback.status,
      to_step_id: stateData.rollback.to_step_id,
      boundary_step_id: stateData.rollback.boundary_step_id,
      conflict: stateData.rollback.conflict,
    } : null,
    latest_receipt_path: stateData.rollbacks?.at(-1)?.receipt_path ?? null,
  };
}
...
async function findCodexState(filePath, registryId = null) {
  const markdownPath = path.resolve(filePath);
  const slug = campaignFileStem(markdownPath);
