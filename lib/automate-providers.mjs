...
import { assertValidRunState } from './run-state.mjs';
...
function automateStatePriority(state) {
  ...
  if (status === 'stalled' || status === 'blocked' || status === 'awaiting_human_review') return 60;
...
function engineProvider() {
  return {
    ...
    async getState(filePath, { summary = false, registryId = null } = {}) {
      ...
      const currentLedgerStep = stateData.run.current_step_id
        ? stateData.steps.find((step) => step.id === stateData.run.current_step_id) ?? null
        : null;
      const currentStep = currentLedgerStep
        ? {
            id: currentLedgerStep.id,
            name: currentLedgerStep.name,
            phase: currentLedgerStep.phase,
            phase_name: currentLedgerStep.phase ? `Phase ${currentLedgerStep.phase}` : null,
            started_at: currentLedgerStep.started_at,
          }
        : null;
      ...
      const currentStepLog = stateData.worker?.log_path
        ? await readTextFile(stateData.worker.log_path)
        : null;
      const steps = await enrichEngineSteps(stateData.steps, campaignMap, stateData.config.runner);
      ...
      return {
        ...
        live_activity: stateData.config.runner === 'claude'
...
export function deriveEngineRuntime(stateData, lock) {
  ...
  const livePid = [stateData.worker?.pid, lock?.pid]
  ...
  if (['stopped', 'halted'].includes(runStatus)) return { status: 'halted', isActive: false };
...
async function enrichEngineSteps(steps, campaignMap, runner) {
...
function automationExecutionContext(filePath, stateData) {
  ...
  const explicitlyWorktree = Boolean(worktree.path || worktree.base_repo_path);
