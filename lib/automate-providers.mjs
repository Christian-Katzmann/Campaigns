  ...
  return {
    ...
    async detect(filePath) {
      ...
      try {
        ...
        return campaignStateMatchesFile(filePath, JSON.parse(raw));
      } catch {
        return false;
      }
    },
    ...
    async getState(filePath, { summary = false } = {}) {
      ...
      if (!(await campaignStateMatchesFile(filePath, stateData))) return null;
...
async function findCodexStatePath(filePath) {
  ...
  try {
    ...
    if (await campaignStateMatchesFile(markdownPath, stateData)) {
      match = statePath;
    }
  } catch (err) {
    if (err.code !== 'ENOENT') {
...
export async function campaignStateMatchesFile(filePath, stateData) {
  ...
  const [registeredIdentity, executionIdentity] = await Promise.all([
...
async function gitFileIdentity(filePath) {
  ...
  try {
    const [rootResult, commonResult] = await Promise.all([
      execFileText('git', ['-C', cwd, 'rev-parse', '--show-toplevel'], { timeout: 5_000 }),
      execFileText(
        'git',
    ...
    const relativePath = relativePathWithin(canonicalRoot, canonicalFile);
    if (!root || !commonDir || !relativePath) return null;
    return {
      commonDir: (await comparablePath(commonDir)).normalize('NFC'),
      relativePath,
    };
  } catch {
    return null;
