...
} from './notification-commands.mjs';
...
import { runFinalReview, runFixAttempt, selectReviewRunner } from './review.mjs';
import {
  createRunState,
  transitionRunState,
} from './run-state.mjs';
...
export class HumanReviewApprovalError extends Error {
  constructor(message) {
    super(message);
    this.name = 'HumanReviewApprovalError';
  }
}
...
export function runPathsForCampaign(campaignFile, runsDir = defaultCampaignsRunsDir()) {
  ...
  const stem = campaignFileStem(campaignPath)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')
    .slice(0, 48) || 'campaign';
  ...
  return {
    ...
    liveOutputDir: path.join(runDir, 'live-output'),
...
export async function requestCampaignStop(campaignFile, options = {}) {
  ...
  let state;
  try {
    state = await readRunState(paths.statePath);
  } catch (error) {
...
export async function approveCampaignHumanReview(campaignFile, options = {}) {
  const env = options.env ?? process.env;
  const campaignPath = await canonicalExistingPath(campaignFile, 'Campaign file');
  const runsDir = path.resolve(options.runsDir ?? defaultCampaignsRunsDir(env));
  const paths = runPathsForCampaign(campaignPath, runsDir);
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      throw new HumanReviewApprovalError(`No run ledger found for ${campaignPath}.`);
    }
    throw error;
  }

  if (typeof options.runId !== 'string' || state.run.id !== options.runId) {
    throw new HumanReviewApprovalError('Approval run id does not match the current run.');
  }
  if (state.run.status !== 'awaiting_human_review') {
    throw new HumanReviewApprovalError(`Campaign run is ${state.run.status}, not awaiting human review.`);
  }
  if (state.history.at(-1)?.event !== 'reviewer_unavailable') {
    throw new HumanReviewApprovalError(
      `Human approval is unavailable after ${state.history.at(-1)?.event ?? 'an unknown transition'}.`,
    );
  }
  ...
  if (!merge.ok) {
    state = transitionRunState(state, {
      ...
      message: `Human-approved review could not merge: ${merge.error}`,
      error: merge.error,
      verdict: 'APPROVED',
    });
    await persistState(paths.statePath, state);
    throw new HumanReviewApprovalError(`Approved work could not merge: ${merge.error}`);
  }

  state = transitionRunState(state, {
    event: 'human_review_approved',
    message: 'A human approved the campaign after the reviewer was unavailable.',
    approved_via: options.approvedVia ?? 'api',
  });
  ...
  if (state.artifacts.worktree && !state.artifacts.worktree.pruned_at) {
    await pruneExecutionWorktree(state, paths.statePath, { deleteBranch: true });
  }
  return { ok: true, campaignPath, statePath: paths.statePath, state, merge };
}
...
export async function runCampaign(campaignFile, options = {}) {
  ...
  let state = null;
  try {
    const runnerRegistry = await loadConfiguredRunnerRegistry(rawConfig);
    for (const warning of [...resolvedConfig.warnings, ...runnerRegistry.warnings]) {
    ...
    const reviewer = effective.reviewer ?? 'auto';
    ...
    const maxCostUsd = positiveNumberOr(
      numberOption(effective.maxCostUsd),
      null,
      'run.max_cost_usd',
    );
    requireCostReportingRunner(runnerRegistry, runnerName, maxCostUsd);
    ...
    await rm(paths.liveOutputDir, { recursive: true, force: true });
    ...
    while (true) {
      ...
      const costCap = reachedCostCap(state);
      if (costCap) {
        state = transitionRunState(state, {
          event: 'cap_reached',
          message: costCapMessage(costCap),
          details: costCapDetails(costCap),
        });
        await persistState(paths.statePath, state);
        stdout?.write(`Dollar cap reached.\nState: ${paths.statePath}\n`);
        return { ...paths, state };
      }
      ...
      if (!parallelGroup.reason && parallelGroup.steps.length > 1 && state.config.max_cost_usd != null) {
        parallelGroup = {
          steps: [nextStep],
          reason: 'max_cost_usd requires sequential runner launches',
        };
      }
      ...
      const liveOutputPath = path.join(
        paths.liveOutputDir,
        `${safeStepId(nextStep.id)}-${invocationId}.json`,
      );
      const expected = {
        run_id: state.run.id,
        step_id: nextStep.id,
      ...
      requireCostReportingRunner(
        runnerRegistry,
        stepSelection.runner,
        state.config.max_cost_usd,
      );
      ...
      const stepBaseOid = await gitHead(repoRoot);
        ...
        onSpawn: async (pid) => {
          state = transitionRunState(state, {
            ...
            worker: {
              ...
              pid,
              log_path: logPath,
              live_output_path: liveOutputPath,
            },
          });
          await persistState(paths.statePath, state);
        },
      ...
      if (result.stop?.requested || result.cap) {
        if (result.stop?.requested && !result.stop.forced) {
          ...
          if (boundaryResult.completed) {
            ...
            const ticked = await tickCampaignStep(paths.statePath, campaignPath, nextStep.id);
            state = transitionRunState(state, {
              ...boundaryResult.transition,
              commit_range: {
                base_oid: stepBaseOid,
                head_oid: await gitHead(repoRoot),
              },
              runner: invocation.runner,
              model: invocation.model,
              effort: invocation.effort,
        ...
        state = transitionRunState(state, {
          ...
          details: {
            ...
            usage: extractRunnerUsage(runnerRegistry, invocation.runner, result.stdout),
          },
        });
        await persistState(paths.statePath, state);
      ...
      const classified = result.watchdog?.stalled
        ? {
            completed: false,
            usage: { ...EMPTY_RUNNER_USAGE },
            transition: {
              event: 'step_failed',
              step_id: nextStep.id,
              details: { usage: { ...EMPTY_RUNNER_USAGE } },
              failure: {
      ...
      if (checkGate.stop?.requested || checkGate.cap) {
        ...
        const message = checkGate.stop?.requested
          ...
          : capMessage(checkGate.cap, ` while fixing checks for Step ${nextStep.id}.`);
        await writeReceipt(receiptPath, {
          runId: state.run.id,
          step: nextStep,
          runner: invocation.runner,
      ...
      if (!checkGate.passed) {
        ...
        state = transitionRunState(state, {
          ...
          details: { usage: classified.usage },
        });
        await persistState(paths.statePath, state);
...
async function runParallelStepGroup({
  ...
  const runJob = async (job, index) => {
    ...
    const liveOutputPath = path.join(
      paths.liveOutputDir,
      `${safeStepId(job.step.id)}-${invocationId}.json`,
    );
    const expected = {
      run_id: state.run.id,
      step_id: job.step.id,
  ...
  for (const outcome of classified.sort((left, right) => (
    ...
    if (!dirty.ok || dirty.stdout.trim()) {
      outcome.classified = parallelFailure(
        ...
        dirty.stdout,
        outcome.classified.usage,
      );
      continue;
    }
    outcome.commitRange = {
      base_oid: phaseBase,
      head_oid: await gitHead(outcome.job.worktree.path),
    };
  ...
  for (const outcome of classified.sort((left, right) => (
    ...
    if (outcome.classified.completed) {
      ...
      const ticked = await tickCampaignStep(paths.statePath, campaignPath, outcome.job.step.id);
      state = transitionRunState(state, {
        ...outcome.classified.transition,
        commit_range: outcome.commitRange,
...
function parallelFailure(stepId, code, message, outputTail, usage = { ...EMPTY_RUNNER_USAGE }) {
  return {
    completed: false,
    usage,
    transition: {
...
async function runCheckFixLoop({
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const costCap = reachedCostCap(state, pendingUsage);
    if (costCap) {
      return {
        state,
        passed: false,
        ...
        fixes,
        cap: costCap,
        unrecordedUsage: pendingUsage,
      };
    }
    requireCostReportingRunner(runnerRegistry, runnerName, state.config.max_cost_usd);
    ...
    fixes.push(fix);
    state = transitionRunState(state, {
      event: 'check_fix_finished',
      step_id: step?.id ?? null,
      message: `Finished ${scope} check fix attempt ${attempt}.`,
      details: {
        scope,
        attempt,
        committed: fix.committed,
        outcome: fix.stop?.requested ? 'stopped' : fix.cap ? 'cap_reached' : 'completed',
        usage: fix.usage,
      },
    });
    await persistState(paths.statePath, state);
    ...
    const postFixCostCap = reachedCostCap(state, pendingUsage);
    if (postFixCostCap) {
      return {
        state,
        passed: false,
...
async function runReviewBoundary({
  ...
  let fixAttempts = 0;
  const runDeadlineMs = Date.parse(state.run.started_at) + state.config.max_run_minutes * 60_000;
  const guardedRunInvocation = (invocation, invocationOptions) => {
    const costCap = reachedCostCap(state);
    if (costCap) return Promise.resolve(invocationBlockedByCap(costCap));
    requireCostReportingRunner(
      runnerRegistry,
      invocation.runner,
      state.config.max_cost_usd,
    );
    return runRunnerInvocation(invocation, {
      ...invocationOptions,
      containmentRoot: repoRoot,
      stopRequestPath: paths.stopRequestPath,
      stopGraceMs: state.config.stop_grace_ms,
  ...
  while (true) {
    ...
    if (costCap) {
      state = transitionRunState(state, {
        event: 'cap_reached',
        message: costCapMessage(costCap, ' before final review.'),
        details: costCapDetails(costCap),
      });
      await persistState(paths.statePath, state);
      return { ...paths, state };
    ...
    if (checkGate.stop?.requested || checkGate.cap) {
      ...
      state = transitionRunState(state, {
        ...
        message: checkGate.stop?.requested
          ...
          : capMessage(checkGate.cap, ' while fixing campaign checks before final review.'),
        details: {
          ...costCapDetails(checkGate.cap),
          forced: checkGate.stop?.forced ?? true,
          grace_ms: checkGate.stop?.grace_ms ?? null,
    ...
    const reviewer = selectReviewRunner({
      capabilities: await runnerCapabilities(runnerRegistry, { cwd: repoRoot, env }),
      workerRunner: state.config.runner,
      reviewer: state.config.reviewer,
    });
    const reviewerMetadata = {
      reviewer_runner: reviewer.runner,
      reviewer_family: reviewer.family,
      reviewer_ladder_tier: reviewer.ladder_tier,
    };
    if (!reviewer.available) {
      state = transitionRunState(state, {
        event: 'reviewer_unavailable',
        message: reviewer.runner
          ? `Configured reviewer ${reviewer.runner} is unavailable; awaiting human review.`
          : 'No configured runner is available for final review; awaiting human review.',
        ...reviewerMetadata,
      });
      await persistState(paths.statePath, state);
      await notifyAwaitingHumanReview(state, env, options);
      stdout?.write(`No automated reviewer is available; awaiting human review.\nState: ${paths.statePath}\n`);
      return { ...paths, state };
    }
    requireCostReportingRunner(
      runnerRegistry,
    ...
    state = transitionRunState(state, {
      ...
      message: `Final review round ${reviewRound} started with ${reviewer.runner}.`,
      ...reviewerMetadata,
    });
    await persistState(paths.statePath, state);
    ...
    review = await runFinalReview({
      ...
      reviewRound,
      registry: runnerRegistry,
      runnerName: reviewer.runner,
      repoRoot,
      paths,
      model: reviewer.defaults.model,
      effort: reviewer.defaults.effort,
      env,
      watchdog: state.config.watchdog,
      signal: options.signal,
      stdout,
    ...
    if (review.kind === 'stopped' || review.kind === 'cap_reached') {
      ...
      state = transitionRunState(state, {
        ...
        message: review.kind === 'stopped'
          ...
          : capMessage(review.cap, ' during final review.'),
        details: {
          ...costCapDetails(review.cap),
          ...
          usage: review.attempts.at(-1)?.usage ?? { ...EMPTY_RUNNER_USAGE },
        },
      });
      await persistState(paths.statePath, state);
    ...
    const reviewUsage = review.attempts.at(-1)?.usage ?? { ...EMPTY_RUNNER_USAGE };
    const reviewCostCap = reachedCostCap(state, [reviewUsage]);
    if (reviewCostCap) {
      state = transitionRunState(state, {
        event: 'cap_reached',
        message: costCapMessage(reviewCostCap, ' during final review.'),
        details: {
          ...costCapDetails(reviewCostCap),
    ...
    while (fixAttempts < (state.config.max_fix_attempts ?? 2)) {
      ...
      if (fix.stop?.requested || fix.cap) {
        ...
        state = transitionRunState(state, {
          ...
          message: fix.stop?.requested
            ...
            : capMessage(fix.cap, ' during final-review rework.'),
          details: {
            ...costCapDetails(fix.cap),
      ...
      const fixCostCap = reachedCostCap(state, [fix.usage]);
      if (fixCostCap) {
        state = transitionRunState(state, {
          event: 'cap_reached',
          message: costCapMessage(fixCostCap, ' during final-review rework.'),
          details: {
            ...costCapDetails(fixCostCap),
            salvage_path: fix.log_path,
            usage: fix.usage,
          },
        });
...
async function notifyAwaitingHumanReview(state, env, options) {
  ...
  let settings;
  const dataDir = defaultCampaignsDataDir(env);
  try {
  ...
  const message = `${campaignFileStem(state.run.identity.execution.campaign_path)} is awaiting human review.`;
  if (notificationPolicyDecision(settings, 'awaiting_human_review', new Date()) === 'digest') {
    const digestPath = path.join(dataDir, 'notification-digest.json');
    let digestState;
    try {
      digestState = sanitizeNotificationDigestState(JSON.parse(await readFile(digestPath, 'utf8')));
    } catch (error) {
      if (error.code !== 'ENOENT' && !(error instanceof SyntaxError)) throw error;
      digestState = sanitizeNotificationDigestState(null);
    }
    digestState = enqueueNotificationDigest(digestState, {
      title,
      message,
      category: 'awaiting_human_review',
      eventKey: `awaiting_human_review:${state.run.id}`,
    });
    await writeFileAtomic(digestPath, `${JSON.stringify(digestState, null, 2)}\n`);
    return;
  }
  const deliveries = [];
  let ntfyActions = [];
  if (NTFY_TOPIC_REGEX.test(settings.ntfyTopic)) {
    const command = await ensureNotificationCommandInfrastructure(dataDir, settings);
    ...
    const approveToken = state.history.at(-1)?.event === 'reviewer_unavailable'
      ? createNotificationCommand({
        action: 'approve',
        runId: state.run.id,
        secret: command.secret,
      ...
      : '';
    ntfyActions = buildNtfyRunActions({
      commandTopic: settings.ntfyCommandTopic,
      stopToken,
      approveToken,
      openUrl: settings.verifiedPhoneUrl,
    });
  }
...
async function loadOrCreateState({
  ...
  try {
    const state = await readRunState(paths.statePath);
...
export function reportedRunCostUsd(state) {
  return (state?.history ?? []).reduce((total, entry) => {
    const cost = entry?.details?.usage?.cost_usd;
    return total + (typeof cost === 'number' && Number.isFinite(cost) ? cost : 0);
  }, 0);
}

function reachedCostCap(state, additionalUsage = []) {
  const limit = state?.config?.max_cost_usd;
  if (limit == null) return null;
  const pending = additionalUsage.reduce((total, usage) => {
    const cost = usage?.cost_usd;
    return total + (typeof cost === 'number' && Number.isFinite(cost) ? cost : 0);
  }, 0);
  const totalCostUsd = reportedRunCostUsd(state) + pending;
  if (totalCostUsd < limit) return null;
  return { kind: 'max_cost_usd', limit, total_cost_usd: totalCostUsd };
}

function requireCostReportingRunner(registry, runnerName, maxCostUsd) {
  if (maxCostUsd == null || runnerReportsCost(registry, runnerName)) return;
  throw new PumpRunError(
    `Runner ${runnerName} cannot run with max_cost_usd because it has no cost_usd usage mapping.`,
  );
}

function costCapDetails(cap) {
  if (!cap) return { cap: null };
  return {
    cap: cap.kind ?? null,
    ...(cap.limit == null ? {} : { limit: cap.limit }),
    ...(cap.total_cost_usd == null ? {} : { total_cost_usd: cap.total_cost_usd }),
  };
}

function costCapMessage(cap, suffix = '.') {
  const total = Number(cap.total_cost_usd).toFixed(6).replace(/0+$/, '').replace(/\.$/, '');
  return `Dollar cap reached at $${total} (limit $${cap.limit})${suffix}`;
}

function capMessage(cap, suffix = '.') {
  return cap?.kind === 'max_cost_usd'
    ? costCapMessage(cap, suffix)
    : `Run-time cap reached${suffix}`;
}

function invocationBlockedByCap(cap) {
  return {
    exitCode: 1,
    stdout: '',
    stderr: '',
...
function combineUsage(usages) {
  const fields = Object.keys(EMPTY_RUNNER_USAGE);
  return Object.fromEntries(fields.map((field) => {
    const values = usages
      .map((usage) => usage?.[field])
      .filter((value) => typeof value === 'number' && Number.isFinite(value));
    return [field, values.length > 0 ? values.reduce((sum, value) => sum + value, 0) : null];
  }));
}
...
async function tickCampaignStep(statePath, campaignPath, stepId) {
