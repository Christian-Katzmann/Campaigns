...
import { runFinalReview } from './review.mjs';
import {
  assertValidRunState,
  createRunState,
  transitionRunState,
...
export async function runCampaign(campaignFile, options = {}) {
  ...
  try {
    ...
    if (['awaiting_human_review', 'completed', 'merged', 'force_merged'].includes(state.run.status)) {
...
async function runReviewBoundary({
  ...
  let state = transitionRunState(initialState, {
    event: 'final_review_started',
    message: 'Final review started.',
  });
  await persistState(paths.statePath, state);
  stdout?.write('Starting final review.\n');
  ...
  } else if (review.kind === 'needs_work') {
    state = transitionRunState(state, {
  ...
  await persistState(paths.statePath, state);
  stdout?.write(`Final review result: ${review.verdict ?? 'UNPARSEABLE'}.\nState: ${paths.statePath}\n`);
