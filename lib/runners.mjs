...
export const EMPTY_RUNNER_USAGE = Object.freeze({
  input_tokens: null,
  output_tokens: null,
  total_tokens: null,
  cost_usd: null,
...
export function classifyRunnerResult(registry, runnerName, result) {
  ...
  const usage = extractUsage(runner.usage, stdout);
...
export function extractRunnerUsage(registry, runnerName, stdout) {
  return extractUsage(registry.get(runnerName).usage, String(stdout ?? ''));
}

export function runnerReportsCost(registry, runnerName) {
  return registry.get(runnerName).usage.sources.some((source) => (
    Object.hasOwn(source.fields, 'cost_usd')
  ));
}
...
function normalizeRunner(name, runner, { requireFamily = false } = {}) {
  ...
  const usage = normalizeUsage(name, runner.usage);
...
function normalizeUsage(name, usage) {
  if (usage == null) return { sources: Object.freeze([]) };
  if (!isObject(usage) || !Array.isArray(usage.sources) || usage.sources.length === 0) {
    throw new TypeError(`runners.${name}.usage.sources must be a non-empty array`);
  }
  const sources = usage.sources.map((source, index) => {
    const label = `runners.${name}.usage.sources[${index}]`;
    if (!isObject(source) || source.kind !== 'jsonl') {
      throw new TypeError(`${label}.kind must be jsonl`);
    }
    if (!isObject(source.match)) throw new TypeError(`${label}.match must be an object`);
    if (!isObject(source.fields)) throw new TypeError(`${label}.fields must be an object`);
    const unknown = Object.keys(source.fields).find((field) => (
      !Object.hasOwn(EMPTY_RUNNER_USAGE, field)
    ));
    if (unknown) throw new TypeError(`${label}.fields.${unknown} is not supported`);
    if (Object.keys(source.fields).length === 0) {
      throw new TypeError(`${label}.fields must name at least one usage field`);
    }
    return Object.freeze({
      kind: 'jsonl',
      match: Object.freeze(normalizeScalarMap(source.match, `${label}.match`)),
      fields: Object.freeze(normalizeStringMap(source.fields, `${label}.fields`)),
    });
  });
...
function extractUsage(usage, stdout) {
  for (const source of usage.sources) {
    for (const line of stdout.split(/\r?\n/)) {
      ...
      try {
        const event = JSON.parse(line);
        if (!isObject(event) || !matchesFields(event, source.match)) continue;
        const normalized = { ...EMPTY_RUNNER_USAGE };
        for (const [field, fieldPath] of Object.entries(source.fields)) {
          normalized[field] = normalizeUsageValue(field, valueAtPath(event, fieldPath));
        }
        ...
        // Non-JSON output and malformed usage values do not break runner completion.
      }
    }
  }
  return { ...EMPTY_RUNNER_USAGE };
}

function normalizeUsageValue(field, value) {
  if (value == null) return null;
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0) {
    throw new TypeError(`${field} must be a non-negative number`);
  }
  if (field.endsWith('_tokens') && !Number.isInteger(value)) {
...
function failedResult(stepId, code, message, stdout, stderr, usage) {
  return {
    completed: false,
    marker: null,
    usage,
